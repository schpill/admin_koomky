<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Quote {{ $quote->number }}</title>
    <style>
        body { font-family: DejaVu Sans, Arial, sans-serif; font-size: 12px; color: #111827; }
        h1 { font-size: 18px; margin: 0 0 8px; }
        h2 { font-size: 14px; margin: 18px 0 8px; }
        table { width: 100%; border-collapse: collapse; margin-top: 10px; }
        th, td { border: 1px solid #d1d5db; padding: 6px; text-align: left; }
        th { background: #f3f4f6; }
        .totals td { font-weight: bold; }
        .meta { margin-top: 6px; }
        .logo { margin-bottom: 10px; }
        .logo img { height: 36px; width: auto; }
    </style>
</head>
<body>
    @if (!empty($logoDataUri))
        <div class="logo">
            <img src="{{ $logoDataUri }}" alt="Company logo">
        </div>
    @endif

    <h1>Quote {{ $quote->number }}</h1>

    <div class="meta">
        <strong>Issue date:</strong> {{ $quote->issue_date?->toDateString() }}<br>
        <strong>Valid until:</strong> {{ $quote->valid_until?->toDateString() }}<br>
        <strong>Status:</strong> {{ $quote->status }}
    </div>

    <h2>From</h2>
    <div>{{ $quote->user?->business_name ?? $quote->user?->name }}</div>

    <h2>Client</h2>
    <div>{{ $quote->client?->name }}</div>
    <div>{{ $quote->client?->email }}</div>

    <h2>Line Items</h2>
    <table>
        <thead>
            <tr>
                <th>Description</th>
                <th>Qty</th>
                <th>Unit Price</th>
                <th>VAT %</th>
                <th>Total</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($quote->lineItems as $lineItem)
                <tr>
                    <td>{{ $lineItem->description }}</td>
                    <td>{{ number_format((float) $lineItem->quantity, 2, '.', '') }}</td>
                    <td>{{ number_format((float) $lineItem->unit_price, 2, '.', '') }}</td>
                    <td>{{ number_format((float) $lineItem->vat_rate, 2, '.', '') }}</td>
                    <td>{{ number_format((float) $lineItem->total, 2, '.', '') }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>

    <table class="totals">
        <tbody>
            <tr>
                <td>Subtotal</td>
                <td>{{ number_format((float) $quote->subtotal, 2, '.', '') }}</td>
            </tr>
            <tr>
                <td>Tax</td>
                <td>{{ number_format((float) $quote->tax_amount, 2, '.', '') }}</td>
            </tr>
            <tr>
                <td>Total</td>
                <td>{{ number_format((float) $quote->total, 2, '.', '') }}</td>
            </tr>
        </tbody>
    </table>

    @if ($quote->notes)
        <h2>Notes</h2>
        <div>{{ $quote->notes }}</div>
    @endif
</body>
</html>

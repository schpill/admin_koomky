...
- **Phase 23 closed & merged to `main`** — Branch `apex/phase23-docs` (merged 2026-03-08) delivers le module Documentation intégrée (v2.9.0) : infrastructure MDX (`@next/mdx` + `remark-gfm` + `rehype-highlight`), routing `app/(dashboard)/docs/[[...slug]]`, 9 composants docs (`DocLayout`, `DocSidebar`, `DocToc`, `DocCallout`, `DocScreenshot`, `DocDiagram`, `DocInfographic`, `DocSteps`, `DocBadge`), script `generate-diagrams.mts` (Gemini API → 10 diagrammes Mermaid + 8 infographies SVG), script Playwright `capture-screenshots.ts`, index de recherche `search-index.json` généré en prebuild + extension CommandPalette (groupe "Documentation"), entrée "Documentation" dans la sidebar. 22 articles MDX complets (getting-started, dashboard, clients, leads, invoices + lifecycle, quotes, credit-notes, expenses, projects, calendar, campaigns + ab-testing, drip, workflows, suppression, warmup, scoring, documents, tickets, rag + mcp, portal, reminders, settings). 6 tests frontend.
- **Phase 22 closed & merged to `main`** — Branch `apex/phase22-ux-remember-me` (PR #36, merged 2026-03-08) delivers UX polish & "Rester connecté" (v2.8.0) : `LoginRequest` + `remember_me` boolean, `AuthController::issueTokens()` émet un refresh token 30 j si `remember_me=true` / 24 h sinon. Frontend : checkbox "Rester connecté.e" pré-cochée sur la page de connexion (cookie session ou durable selon valeur), `auth.ts` store aligné sur la persistance session vs durable. `NotificationBell` entièrement i18née (`notifications.*` FR + EN, suppression des textes hardcodés anglais). Double layout corrigé sur `/profile` (retrait du wrapper `<DashboardLayout>` redondant). `cursor-pointer` ajouté sur tous les boutons icônes du header. Wording login : "Bon retour" → "Connexion" (FR) / "Welcome back" → "Sign in" (EN). 2 tests backend + 3 tests frontend.
- **Phase 21 closed & merged to `main`** — Branch `apex/phase20` (merged 2026-03-07) delivers user menu & profile settings (v2.7.0): `ProfileController` (`GET /profile`, `PATCH /profile` avec upload avatar, `POST /profile/password` + révocation des autres tokens Sanctum), `UpdateProfileRequest` (unicité email ignore self, avatar image max 2048), `UpdatePasswordRequest` (vérification `current_password` + Hash::check), colonne `avatar_path` VARCHAR(500) nullable dans `users`, accessor `avatar_url` (Storage::disk('public')->url). Frontend : `header.tsx` migré vers `DropdownMenu` shadcn/ui (trigger avatar/initiales, items "Mon profil" → `/profile` + "Déconnexion"), page `app/(dashboard)/profile/page.tsx` (section infos personnelles + section mot de passe, toasts succès/erreur), composant `AvatarUpload` (drag/drop + preview + suppression), store Zustand `profile.ts` (fetchProfile/updateProfile/changePassword avec sync authStore). 6 tests backend + 7 tests frontend.
- **Phase 20 closed & merged to `main`** — Branch `apex/phase20` (merged 2026-03-07) delivers preference center RGPD, timezone STO & rapports exportables (v2.6.0): table `communication_preferences` (UUID PK, contact FK CASCADE, category ENUM newsletter/promotional/transactional, subscribed), `PreferenceCenterService` (getPreferences/updatePreference/isAllowed, crée les 3 préférences par défaut), `PreferenceCenterController` (GET+POST `/portal/preferences/{contact}`, URL signée 30 jours), filtre préférences dans `SendEmailCampaignJob`/`SendDripStepEmailJob`/`SendWorkflowEmailJob` (transactionnel toujours autorisé), variable `{{preferences_url}}` dans `PersonalizationService`. Timezone STO : colonne `timezone` VARCHAR(64) sur `Contact`, `ContactSendTimeService` timezone-aware (`AT TIME ZONE` SQL, fallback UTC), détection automatique via IP dans `EmailTrackingController`. Rapports : `CampaignReportService` (getFullReport/exportCsv/exportPdf via dompdf), vue Blade `reports/campaign-report.blade.php`, endpoints `/report` + `/report/pdf` + `/report/csv` dans `CampaignController`. `DeliverabilityScoreService` (score 0–100, règles : mots spam, ratio texte/images, lien désabonnement, longueur sujet, majuscules, alt images, HTML invalide). GDPR : `DataExportService` inclut `CommunicationPreference`. Frontend : page portail `/portal/preferences/[contact]`, `PreferenceCenterForm` (3 toggles + unsubscribe all), `DeliverabilityBadge` (vert/orange/rouge + popover issues), boutons export PDF/CSV + timeline LineChart dans analytics, champ timezone (select IANA) dans fiche contact, store `campaigns.ts` étendu (`email_category` + `deliverability`). 8 tests backend + 5 tests frontend + 2 E2E.

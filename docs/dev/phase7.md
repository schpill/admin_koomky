...
> **Status**: Completed (PR #18 merged to `main` on 2026-02-20)
...
| P7-BE-026 | Create WebhookDispatchJob (queued, exponential backoff retry, max 5 attempts) | merged | |
| P7-BE-027 | Create WebhookDeliveryController (delivery log, manual retry) | merged | |
| P7-BE-028 | Integrate WebhookDispatchService into all documented application events (14 events) | merged | |
| P7-BE-029 | Integrate dedoc/scramble to auto-generate OpenAPI 3.1 spec at /api/docs | merged | |
| P7-BE-030 | Scope-guard middleware: enforce PAT abilities on API routes | merged | |
...
| P7-FE-023 | Create components/settings/webhook-form.tsx (URL + event checkboxes) | merged | |
| P7-FE-024 | Create components/settings/api-token-form.tsx (name + scope checkboxes + expiry) | merged | |
...
| P7-BE-045 | Create StoreLeadRequest (validation: email, phone E.164, probability, value) | merged | |
...
| P7-BE-052 | Dispatch webhooks for lead events (lead.created, lead.status_changed, lead.converted) | merged | |
| P7-BE-053 | Configure Meilisearch index for Lead | merged | |
| P7-BE-054 | Add leads to DataExportService (GDPR export) | merged | |
...
| P7-FE-036 | Create components/leads/lead-kanban.tsx (dnd-kit Kanban board) | merged | |
| P7-FE-037 | Create components/leads/lead-activity-form.tsx (inline activity logger) | merged | |
| P7-FE-038 | Create components/leads/convert-to-client-dialog.tsx (conversion confirmation) | merged | |
| P7-FE-039 | Add Leads entry to sidebar navigation (above Clients, with pipeline value badge) | merged | |
| P7-FE-040 | Add pipeline summary widget to dashboard (total value, funnel mini, win rate) | merged | |

...
test.describe("Portal Payment Flow", () => {
  test("user can complete a payment workflow", async ({ page }) => {
    ...
    await page.route("https://api.stripe.com/v1/payment_intents/**/confirm",
      async (route) => {
    ...
    await page.route("**/api/v1/portal/invoices/inv_1", (route) => route.fulfill({
        status: 200,
        contentType: "application/json",
        body: portalResponse({ id: 'inv_1', number: 'INV-001', total: 120, currency: 'EUR', status: 'sent' })
    }));

    await page.goto("/portal/invoices/inv_1/pay");

    await expect(page.getByRole("heading", { name: "Pay Invoice INV-001" })).toBeVisible();
    ...
    await expect(page.getByText("Stripe publishable key is missing")).not.toBeVisible();
    ...
    await page.waitForResponse("https://api.stripe.com/v1/payment_intents/**/confirm");

...
describe('ClientForm', () => {
  test('renders correctly with initial data', () => {
    ...
    render(<ClientForm initialData={initialData} onSubmit={async () => {}} onCancel={() => {}} />)
    
    expect(screen.getByLabelText(/Company\/Client Name/i)).toHaveValue('Acme Corp')
  ...
  test('calls onSubmit when form is valid', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Company\/Client Name/i), { target: { value: 'New Client' } })
  ...
  test('shows error message when name is too short', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Company\/Client Name/i), { target: { value: 'A' } })

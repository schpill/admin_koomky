...
interface ProspectImportState {
  ...
  updateOptions: (options: Partial<ImportOptions> & { default_tags?: string[] }) => Promise<void>;
...
export const useProspectImportStore = create<ProspectImportState>((set, get) => ({
  session: null,
  columnList: [],
  previewRows: [],
  detectedMapping: {},
  ...
  uploadFile: async (file) => {
    ...
    try {
      ...
      const response = await apiClient.post<any>("/import-sessions", formData);
  ...
  pollStatus: async () => {
    ...
    while (keepPolling) {
      const response = await apiClient.get<any>(`/import-sessions/${session.id}`);
  ...
  fetchErrors: async (page = 1) => {
    ...
    const response = await apiClient.get<any>(`/import-sessions/${session.id}/errors`, {
      params: { page },
    });
  ...
  exportErrors: async () => {
    ...
    const response = await apiClient.get<Blob>(`/import-sessions/${session.id}/errors/export`, {
      responseType: "blob",
    });

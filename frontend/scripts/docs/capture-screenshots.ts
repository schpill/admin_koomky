...
async function main() {
  if (!email || !password) {
    console.log("Skipping screenshots: missing DOCS_SCREENSHOT_EMAIL or DOCS_SCREENSHOT_PASSWORD");
    return;
  }

  const browser = await chromium.launch();
  const page = await browser.newPage({ baseURL, viewport: { width: 1600, height: 1000 } });
  ...
  for (const module of DOC_MODULES) {
    const targetDir = path.join(process.cwd(), "public", "docs", "screenshots", module.slug);
